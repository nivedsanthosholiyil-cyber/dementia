...
import { listAuthorizedPatients, updatePatient } from '@/services/patientService';
...
export function Profiles() {
  ...
  const saveCloud = async () => {
    ...
    try {
      const updated = await updatePatient(activeCloudPatient.id, { name, date_of_birth: numericAge === null ? null : dateOfBirthFromAge(numericAge), notes, interests });
      setCloudList((current) => current.map((patient) => patient.id === updated.id ? updated : patient));
      updateActiveProfile({ id: updated.id, patientName: updated.name });
  ...
  const saveLocal = async () => {
    ...
    if (editingId) {
      ...
      if (settings.activePatientId === updated.id) updateActiveProfile({ patientName: updated.name });
    } else {
      const created = await createProfile(name, details);
  ...
  const select = (profile: PatientProfile) => updateActiveProfile({ id: profile.id, patientName: profile.name });
  const edit = (profile: PatientProfile) => { setEditingId(profile.id); setName(profile.name); setAge(String(ageFromDateOfBirth(profile.dateOfBirth) ?? '')); setNotes(profile.notes ?? ''); setInterests(profile.interests ?? ''); };
  const erase = async (profile: PatientProfile) => { if (!window.confirm(t('profiles.deleteConfirm', { name: profile.name }))) return; await removeProfile(profile.id); if (settings.activePatientId === profile.id) updateActiveProfile({ id: 'local-profile', patientName: '' }); void reloadLocal(); };
    ...
    <main className="page stack">
      ...
      {cloudMode ? <>
        ...
        <div className="card stack-sm"><label className="field__label" htmlFor="profile-name">{t('profiles.newName')}</label><input id="profile-name" className="input" value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" />
          <div className="grid-2"><div><label className="field__label" htmlFor="profile-age">{t('profiles.age')}</label><input id="profile-age" className="input" value={age} onChange={(event) => setAge(event.target.value.replace(/\D/g, '').slice(0, 3))} type="number" min="1" max="120" inputMode="numeric" placeholder={t('profiles.ageOptional')} /></div><div><label className="field__label" htmlFor="profile-interests">{t('profiles.interests')}</label><input id="profile-interests" className="input" value={interests} onChange={(event) => setInterests(event.target.value)} placeholder={t('profiles.interestsPlaceholder')} /></div></div>
          <label className="field__label" htmlFor="profile-notes">{t('profiles.notes')}</label><textarea id="profile-notes" className="input" rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder={t('profiles.notesPlaceholder')} /><small className="muted">{t('profiles.ageHint')}</small><div className="row"><Button block onClick={() => void saveLocal()}>{editingId ? t('profiles.save') : t('profiles.add')}</Button>{editingId && <Button variant="ghost" onClick={resetForm}>{t('profiles.cancel')}</Button>}</div>
        </div>
