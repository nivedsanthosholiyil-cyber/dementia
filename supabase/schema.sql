...
create or replace function public.consume_auth_rate_limit(subject_key text, action_name text, max_attempts integer, window_seconds integer, lock_seconds integer)
...
begin
  if length(subject_key) < 32 or max_attempts < 1 or window_seconds < 1 or lock_seconds < 1 then raise exception 'Invalid rate limit request'; end if;
  insert into private.auth_rate_limits (subject_key, action, attempts) values (subject_key, action_name, 0) on conflict (subject_key, action) do nothing;
  select * into limit_row from private.auth_rate_limits where private.auth_rate_limits.subject_key = consume_auth_rate_limit.subject_key and private.auth_rate_limits.action = consume_auth_rate_limit.action_name for update;
  ...
  if limit_row.window_started_at <= now() - make_interval(secs => window_seconds) then
    update private.auth_rate_limits set attempts = 1, window_started_at = now(), locked_until = null, updated_at = now() where private.auth_rate_limits.subject_key = consume_auth_rate_limit.subject_key and private.auth_rate_limits.action = consume_auth_rate_limit.action_name;
    return true;
  end if;
  if limit_row.attempts >= max_attempts then
    update private.auth_rate_limits set attempts = attempts + 1, locked_until = now() + make_interval(secs => lock_seconds), updated_at = now() where private.auth_rate_limits.subject_key = consume_auth_rate_limit.subject_key and private.auth_rate_limits.action = consume_auth_rate_limit.action_name;
    return false;
  end if;
  update private.auth_rate_limits set attempts = attempts + 1, updated_at = now() where private.auth_rate_limits.subject_key = consume_auth_rate_limit.subject_key and private.auth_rate_limits.action = consume_auth_rate_limit.action_name;
  return true;
end;
$$;
...
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  if selected_language not in ('en', 'hi', 'as', 'bn', 'lus', 'mni') then raise exception 'Unsupported language'; end if;
